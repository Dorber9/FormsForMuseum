...
// add, update, get Quests
app.get("/Quest/:id", (req, res) => {
    const id = req.params.id;
    db.query("SELECT * FROM quest WHERE qid = ?", id, (err, result) => {
        if (err) {
            console.log(err);
        } else {
            res.send(result);
...
app.post("/addQuest", (req, res) => {
    ...
    db.query(
        "INSERT INTO quest (questName,questions,questItems) VALUES (?,?,?)", [questName, questions, itemNames],
        (err, result) => {
            if (err) {
                console.log(err);
            } else {
...
app.put("/updateQuest", (req, res) => {
    const id = req.body.id;
    const questName = req.body.questName;
    const questions = req.body.questions;
    db.query(
        "UPDATE quest SET questName = ?,questions = ? WHERE qid = ?", [questName, questions, id],
        (err, result) => {
            if (err) {
                console.log(err);
            } else {
...
app.delete("/deleteQuest/:id", (req, res) => {
    const id = req.params.id;
    db.query("DELETE FROM quest WHERE qid = ?", id, (err, result) => {
        if (err) {
            console.log(err);
        } else {
            res.send(result);
...
app.get("/ItemShowcase/:id", (req, res) => {
  const id = req.params.id;
  db.query(
    "SELECT ShowcaseID FROM item WHERE ItemName = ?",
    id,
    (err, result) => {
      if (err) {
        console.log(err);
...
app.post("/addQuestion", (req, res) => {
    ...
    questions.forEach((element) => {
        ...
        console.log(element);
        db.query(
            "INSERT INTO Questions (Question,a1,a2,a3,a4,Clue,Correct,ObjectID) VALUES (?,?,?,?,?,?,?,?)", [question, answer1, answer2, answer3, answer4, hint, correct, ObjectID],
            (err, result) => {
                if (err) {
                    console.log(err);
                } else {
