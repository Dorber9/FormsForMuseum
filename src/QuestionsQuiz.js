...
const QuestionsQuiz = () => {
    ...
    useEffect(() => {
        Axios.get(`http://127.0.01:3001/Quest/${params.id}`).then(
            (response) => {
                getQuestions(
    ...
    const getQuestions = (course, title, items) => {
        ...
        arr.forEach((element) => {
            var showcase;
            Axios.get(`http://127.0.01:3001/question/${element}`)
                .then((response) => {
                ...
                .then(
                    Axios.get(
                        `http://127.0.01:3001/ItemShowcase/${itemsArr[i]}`
                    ).then((response2) => {
    ...
    const buildNextItem = (item) => {
        let ShowcaseID = item;
        // Axios.get(`http://127.0.01:3001/ItemShowcase/${item}`).then(
        //   (response) => {
